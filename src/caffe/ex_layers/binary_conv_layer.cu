...
void BinaryConvolutionLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) { //权重可学习
    weight = this->blobs_[0]->gpu_data();
    weight_diff = this->blobs_[0]->mutable_gpu_diff();
  }
  Dtype* bias_diff = NULL;
  if (this->bias_term_ && this->param_propagate_down_[1]) { //bias可学习
  ...
  for (int i = 0; i < top.size(); ++i) {
    ...
    for (int g = 0; g < this->group_; g++) {
      // Gradient w.r.t. bias.
      if (this->bias_term_ && this->param_propagate_down_[1]) { //反向传播计算bias
      ...
      if (this->param_propagate_down_[0]) {
        const Dtype* bottom_data = bottom[i]->gpu_data();
        CUDNN_CHECK(cudnnConvolutionBackwardFilter( //反向传播计算权重
      ...
      // Gradient w.r.t. bottom data.  bottom 数据
      if (propagate_down[i]) {
        if (weight == NULL) {
          weight = this->blobs_[0]->gpu_data();
        }
  ...
  copyGPUTo(W_buffer, this->blobs_[0]); //恢复全精度的数据到blob_[0]
}
