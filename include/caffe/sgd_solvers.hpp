...
class SGDSolver : public Solver<Dtype> {
 ...
 protected:
  ...
  virtual void Regularize(int param_id);//规整化
  virtual void ComputeUpdateValue(int param_id, Dtype rate);//计算某权值在特定学习速率下的更新值
  virtual void ClipGradients();//梯度抑制
  ...
  //SGD求解时需要的临时存储，history_中保留历史增量数据；update_中保留更新相关数据，不需要打快照；
  //temp_保留在计算梯度/更新值时可能需要的其他信息，不需要打快照
