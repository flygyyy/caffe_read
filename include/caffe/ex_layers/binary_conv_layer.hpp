#ifndef CAFFE_BINARY_CONVOLUTION_LAYER_HPP_
#define CAFFE_BINARY_CONVOLUTION_LAYER_HPP_
...
// Binary卷积层 
// 按任务划分 bianactiv_layer实现卷积层输入的二值化，得到sign(I) * K， 得到了两个top blob 一个为sign(I)用于二位卷积计算
// bianary_conv层计算sign(I)与sign(W)的卷积
namespace caffe {
...
class BinaryConvolutionLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  virtual void binarizeCPUTo(const shared_ptr<Blob<Dtype> > weights, const shared_ptr<Blob<Dtype> > wb); //权重二值化
...
#endif  // CAFFE_BINARY_BASE_CONVOLUTION_LAYER_HPP_
