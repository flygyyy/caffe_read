...
class InputLayer : public Layer<Dtype> {
 public:
  ...
  // Data layers have no bottoms, so reshaping is trivial. 数据层没有bottom Blob，reshaping是不重要的
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {}

  virtual inline const char* type() const { return "Input"; }
  virtual inline int ExactNumBottomBlobs() const { return 0; } //bottom为0
  virtual inline int MinTopBlobs() const { return 1; }//top为1
