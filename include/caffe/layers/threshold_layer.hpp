...
class ThresholdLayer : public NeuronLayer<Dtype> {
 public:
  /**
   * @param param provides ThresholdParameter threshold_param, #-参数 threshold_param
   *     with ThresholdLayer options:
   *   - threshold (\b optional, default 0).  可选，默认是0
   *     the threshold value @f$ t @f$ to which the input values are compared.  t是要比较的门限
   */
