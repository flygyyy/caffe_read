...
//变形层：将任意维度的input Blob修改为指定size的输出Blob
//Note：与FlattenLayer相似，这一层不改变输入值
namespace caffe {
...
class ReshapeLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
      const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {}
 //Blob包括四个轴nxcxhxw，如果为0表示与原维度一致，不为0，表示变为此维度，为-1表示需要推断

  /// @brief vector of axes indices whose dimensions we'll copy from the bottom 从bottom拷贝的axes
