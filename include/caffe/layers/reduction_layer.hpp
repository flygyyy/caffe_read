...
//根据任意尺寸的输入得到标量输出，如和，绝对值的和，或者平方和
namespace caffe {
...
class ReductionLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  /// @brief the reduction operation performed by the layer  层操作类型
  ReductionParameter_ReductionOp op_;
  /// @brief a scalar coefficient applied to all outputs  所有输出的标量因子
  Dtype coeff_;
  /// @brief the index of the first input axis to reduce  减少的输入轴索引
  int axis_;
  /// @brief the number of reductions performed  减少的数量
  int num_;
